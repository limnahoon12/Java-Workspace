...
public class A_if {
	...
	public void method1() {
		 ...
		 * => 조건식의 결과가 거짓일 경우 중광호안의 코드가 무시되고 넘어감.
		 * 
		 */
		...
		if(num>0) {
			System.out.println("양수입니다.");// 양수입니다. 출력먼저 적고  if 블록으로 감싼다.
		}
		if(num<=0) {
	...
	public void method3() {
		...
		if(num > 0) {
			...
			String result; //영역이 벗어나기때문에 박스를 먼저 만들기
	...
	public void method5() {
		...
		if(gender =='m' || gender =='M') {
			...
			//return이라는 녀석을 사용
			return;// 해당 일반 메서드를 빠져나감(메소드를 호출했던 곳으로 돌아감.)
		}
	...
	public void method6() {
		...
		//사용자에게 이름을 입력받아 임나훈과 일치하는지 비교하기.
		...
		//String은 참조자료형 
		if(name == "임나훈") {
			System.out.println("임나훈님 반갑습니다.");
		}else {
			System.out.println("임나훈님이 아니신가보네요 ?");
		}
		/*
		 * 기본자료형 : boolean, byte, short, int, long, float, double, char
		 * 참조자료형 : String //특이한 녀석 
		 * 
		 * 기본자료형 간 비교할때는 == != 사용가능하지만, 참조자료형은 동등비교연산시 정상적으로 연산이 안됨.
		 * 그럼 어떻게하냐? 문자열.equals()라는 방법으로 비교를함.
		 * 
		 */
		
		
		if(name.equals("임나훈")) { // 즉 반환값이 true or false로 리턴해줌.
			System.out.println("임나훈님이 반갑습니다.");
		}else {
			System.out.println("임나훈님이 아니신가보네요 ?");
		}
