...
public class Run {
	 ...
	 *  				위헤 배열을 복사해서 새롭게 추가해야함. 즉, **유지보수**가 힘듬.
	 *  
	...
	public static void main(String[] args) {
			...
			hong.name = "홍길동";
			hong.age = 20;
			hong.height = 166.7; // 필드 접근하고자 한다면 . 을통해 가져올수있음(단, 접근제한자가 public일 경우에만)
			...
			System.out.println(hong.name +"님의 나이는"+hong.age+ "살이고" +hong.height+"cm입니다");
			System.out.printf("%s 님의 나이는 %d살이고, 키는 %.1f입니다.", min.name, min.age, min.height);
			
			/*
			 * 필드 public으로 작성시
			 * => 직접적으로 접근해서 값 대입 및 조회 할 경우 보안 문제가 생길 수 있다.
			 * => 외부에서 함부로 값을 변질시키거나 조회권한이 없는 사람이 값을 쉽게 가져울 수 있는 문제발생.
			 * >>> 캡슐화로 해결.
			 * 객채지향의 설계원칙중 하나가 바로 정보은닉이며 그 기술 중 하나가 바로 캡슐화
			 * 
			 */
	}
