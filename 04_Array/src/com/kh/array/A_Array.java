...
public class A_Array {
	 ...
	 *  			a = 20;
	 *  
	 *  배열 : 하나의 공간에 여러개의 값을 담을 수 가 있음. (단, 같은 자료형의 값으로만 가능)
	...
	public void method1() {
		...
		/*
		 * 변수로 이용하여 각각의 데이터를 보관하고자 할 때 => 각각의 변수를 만들어서 따로 관리해야함.
		 * 
		 * int num1=0; 
		 * int num2=1; 
		 * int num3=2, 
		...
		System.out.println(arr); //실제 arr이라는 값에는 조수값이 담겨있음.
	...
	public void method2() {
		...
		//배열 자료형
		/*
		 * 우선 대입연산자 기준 왼쪽이 먼저 실행되므로 arr이라는 int형배열 박스가 먼저 만들어짐.(stack(변수)영역에)
		 * 그 다음, int[5]짜리 방이 heap영역에 생성됨.(new키워드가 붙은녀석은 항상 heap(참조자료형)영역에 할당됨)
		 * 5칸짜리 방이 만들어진후에 각방에 기본값으로(int 0으로)초기화되고, 주소값이 할당됨. (0x123)
		 * 마지막으로 대입연산자가 실행되면서 주소값이 arr에 할당이 됨.
		 * 
		 */
		System.out.println("i : "+i);
		System.out.println("arr : "+arr);	
		System.out.println("arr의 해시코드값 : "+arr.hashCode()); //해시코드 : 주소값을 10진수의 형태로 변환한것.
	...
	public void method3() {
		 ...
		 * -> heap라는 영역에는 절대로 빈공간이 존재할수 없기때문.
	...
	public void method5() {
		...
		//int i2 = null; typeMissMatch -> null값을 int자료형에 넣울 수 없음.
	...
	public void method6() {
		...
		//방법3.
          ...
          /*
           * 항상 고유한 주소값 부여, 기존에 생성되어있던주소와 절대로 겹치지 않는다. 
          ...
          //현재 연결되어있는 연결고리를 끊고자 할 떄
          arr = null; //null : 아무것도 존재하지 않음을 의미함.
	...
	public void method7() {
		...
		// 주소값 == 주소값 : 주소값간에 동등비교를 수행하기때문에 false가 나옴.
	...
	public void method8() {
		...
		//3. 반복문을 활용해서 0번 인덱스부터 마지막 인덱스까지 배열에 담긴갑 출력.
		// 출력예시) arr[x] : xx
...
//		}
	...
	public void method9() {
		...
		public void method10() {
			...
			// 3. 문자열을 각 인덱스별로 뽑아서  char배열의 각 인덱스에 담기
			// arr[0] = 문자열.charAt(0);
	...
	public void method11() {
		//1. 사용자에게 배열의 길이 입력받은 후, 해당 크기만큼 정수배열생성
		Scanner sc = new Scanner(System.in);
		...
		//3. 반복문 활용해서 0번 인덱스부터~마지막인덱스까지 배열에 담긴값 출력과 동시에, 해당 인덱스에 담긴값이 짝수인 경우 값들의 총합 구하기.
		int sum = 0;
		for(int i = 0; i<arr.length; i++) {
	...
	public void method12() {
		...
		// 진행히면서 사용자로부터 키에 대한정보(cm)를 입력받아 각 인덱스에 저장
		Scanner sc = new Scanner(System.in);
		...
		System.out.printf("3명의 총합은 : %.1f, 3명의 평균은 : %.1f \n", sum, (sum/3));
		
		//2. 3명의 키에대한 정보를 출력시키면서 동시에 모든 키에대한 총합 저장
		
		//3. 3명의 총합을 출력하고, 3명키의 평균 출력, 단 소숫점 첫째자리까지 츌력
