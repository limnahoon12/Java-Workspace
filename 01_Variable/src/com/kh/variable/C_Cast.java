...
public class C_Cast {
		 ...
		 * 
		 * 컴퓨터 상에서의 값 처리 규칙
		 * 1. 대입연산자(=)를 기준으로 왼쪽과 오른쪽은 같은 자료형이여함.
		 ...
		 * 
		 * 형변환의 종류
		 * 1. 자동형변환 : -> 자동으로 형변환이 진행되기 떄문에 내가 직업 형변환할 필요가 없다.
		 *				=> 작은바이트의 자료형 -> 큰 바이트의 자료형				 
		 * 
		 * 2. 강제(명시적)형변환 => 자동형변환이 되지 않아 사용자가 직업 강제로 형변환을 해야하는 경우.
	...
	//자동형변환 : 작은바이트의 자료형이 큰 바이트의 자료형으로 형변환 되는경우.
	public void autocasting() {
		
		//1. int (4byte) -> double(8byte)
		int i1 = 10;
		double d1 = (double) i1;// 자동으로 형변환이 되었음.
								// 컴퓨터 포장용 상자에 휴댜폰을 담은 꼴과 비슷하다.
								// int는 정수, double은 실수자료형임.
		...
		double result = /*(double)*/i2 + d2; //(앞에 double가 자동으로 형변환이 되어 들어가 있음)
		...
		long l4 = 2000; //L을 붙이지 않아도 문제가 없는 이유 : 자동형변환이 되기떄문
		
		// 3. 특이케이스 long(8byte) -> float(4byte)
		// 정수가 실수로 담길떄에는 큰 사이즈의 정수여도 작은사이즈의 실수변수에 대입이 가능하다.
		// 아무리 4byte float라고 해도 long형보다 담을 수 있는 값의 범위가 더 크기떄문에
		// float형이 실수이기 떄문에 long형보다 표현가능한 범위의수가 더 커서 자동형변환이 이루어짐.
		...
		/*
		 * char의 범위는 0~65xxx : 각 문자마다 고유의 정수값이 정해져있음.따라서 쌍방향 변환이 가능.
		 * 음수값은 오류가 난다.
		 * 참고 : 아스키코드표(0~127, 영문자, 대소문자, 숫자, 특수문자)
		 * 		유니코드표(0~65xxxx, 영어, 숫자, 특수문자, 한글, 그 외 언어)
		 * 
		 * 
		 */
	...
	public void forceCasting() {
		...
		// iNum이 doble형으로 우선 변환이됨. 10-> 10.0
		...
		// 히결방법 2. 덧셈 연산전에 dNum변수를 미리 int형으로 강제 형변환.
		//			 자동형변환은 작은 바이트변수가 큰 바이트 변수로 형변환 되기 떄문에
		//			 큰 바이트인 dNum변수를 강제로 int형으로 변환시킨것.
